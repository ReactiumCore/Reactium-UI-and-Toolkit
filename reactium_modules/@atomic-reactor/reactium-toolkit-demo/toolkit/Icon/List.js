...
export const List = () => {
    ...
    const icons = useCallback(() => {
        ...
        search = String(search).replace(`${iconSet}.`, '').toLowerCase();
        const list = Object.keys(
            op.get(Icon.Library.listById, [iconSet, 'icons'], {}),
        ).sort();

        console.log('IconLibrary', Icon.Library.list);

        return list.filter((icon) =>
            Boolean(String(icon).toLowerCase().indexOf(search) > -1),
        );
    }, []);
    ...
    return (
        <Element className='p-xs-40' title={__('Icon')}>
            ...
            <div className='rtk-search-form rtk-icon-search-form'>
...
export const IconButton = ({ color, size, value }) => {
    const { Check, Icon } = useHookComponent('RUI');
