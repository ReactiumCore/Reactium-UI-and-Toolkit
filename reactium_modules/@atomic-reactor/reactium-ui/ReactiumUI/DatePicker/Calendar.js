...
import { Feather } from '@atomic-reactor/reactium-ui/Icon';
...
const DaysInMonth = (d, expanded) => {
    const startDay = !expanded
        ? moment(d).clone().startOf('week')
        : moment(d).clone().startOf('month').startOf('week');

    const endDay = !expanded
        ? moment(d).clone().endOf('week')
        : moment(d).clone().endOf('month').endOf('week');
...
let Calendar = ({ namespace, ...props }, ref) => {
    ...
    const setState = (newState, caller) => {
        if (op.has(newState, 'value')) {
            ...
            if (range) {
                ...
                const v = newState.value.split(' ').join('').split(',');
                v.sort();
    ...
    const _next = (duration = 'months') => {
        ...
        date = moment(date).add(1, duration).toDate();
    ...
    const _prev = (duration = 'months') => {
        ...
        date = moment(date).subtract(1, duration).toDate();
