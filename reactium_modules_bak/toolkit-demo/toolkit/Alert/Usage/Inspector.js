...
const Inspector = ({ handle }) => {
  ...
  const { Breakpoint, Toggle } = useHookComponent("ReactiumUI");

  const ScrollWrap = useCallback(
    ({ children }) => (
      <Breakpoint xs={children} lg={<Scrollbars>{children}</Scrollbars>} />
    ),
    []
  );
  ...
  return (
    <ScrollWrap>
      <div className={cn(cx("component-props"), "p-xs-40")}>
        ...
        <div className="form-group">
          <div className="flex middle">
            <div className="col-xs-12 col-sm-6">{__("Message")}</div>
            <div className="col-xs-12 col-sm-6 text-right">
              <input
                type="text"
        ...
        <div className="form-group">
          <Toggle
            ...
            onChange={e => setAttributes({ dismissible: e.target.checked })}
          />
        </div>
      </div>
    </ScrollWrap>
